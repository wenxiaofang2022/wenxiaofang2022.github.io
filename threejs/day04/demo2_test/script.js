...
THREE.OrbitControls = function ( object, domElement ) {
    ...
    // Mouse buttons
    this.mouseButtons = { LEFT: THREE.MOUSE.RIGHT, MIDDLE: THREE.MOUSE.MIDDLE, RIGHT: THREE.MOUSE.RIGHT };
...
var directionalLight = new THREE.DirectionalLight( 0xba8b8b, 1.0 );
directionalLight.position.set( 1, 1, 1 );
scene.add( directionalLight );

var directionalLight2 = new THREE.DirectionalLight( 0x8bbab4, 1.6 );
...
var texture1 = textureLoader.load("./sk-uv-new.png");
...
function createParticles() {
    ...
    // loader.load( "./snorlax_222.glb", function ( gltf ) {
    //     console.log("gltf",gltf);
    //     geometry = gltf.scene.children[0].geometry;
    //     var tmp = gltf.scene.children[0].geometry.attributes.uv.data.array;
    //     console.log("tmp",tmp);
    //     geometry.vertices = tmp;
    //     geometry.faces = [];
    ...
    //         for( j = 0; j < particles[i].adj.length; j++) {
    //             constraints.push( [
    //                 particles[ i ],
...
var GRAVITY = 981 * 1.4;
var gravity = new THREE.Vector3( 0, - 0.98, 0 ).multiplyScalar( 0.1 );
...
function simulate( ) {
    ...
    for (i = 0, il = particles.length; i < il; i ++ ) {
        ...
        var force = new THREE.Vector3().copy (particle.original );
    ...
    for (j = 0; j < 5; j++) {
        ...
        if ( click && psel ) {
            ...
            for ( i = 0; i < particles.length; i++ ) {
                distance = particles[psel].original.distanceTo( particles[i].original );
...
function updateMouse() {
    ...
    newPlane = new THREE.Plane( camera.position.clone().normalize(), -100 );
